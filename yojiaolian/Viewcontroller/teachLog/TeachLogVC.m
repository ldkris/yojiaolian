...
@interface TeachLogVC ()

@end

@implementation TeachLogVC
...
{
    [MobClick endLogPageView:@"教学日志"];
}
-(void)viewWillAppear:(BOOL)animated
{
    [MobClick beginLogPageView:@"教学日志"];
}
-(void)reloadAllData
{
    self.pageIndex=1;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag==0)
    {
        ...
        {
            TeachLogUndoCell *cell=[tableView dequeueReusableCellWithIdentifier:@"TeachLogUndoCell"];
            if (cell==nil)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"TeachLogUndoCell" owner:nil options:nil] objectAtIndex:0];
                cell.delegate=self;
            }
    ...
    else
    {
        ...
        {
            TeachLogDoneCell *cell=[tableView dequeueReusableCellWithIdentifier:@"TeachLogDoneCell"];
            if (cell==nil)
            {
                cell=[[[NSBundle mainBundle] loadNibNamed:@"TeachLogDoneCell" owner:nil options:nil] objectAtIndex:0];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        TeachLogDetailVC *vc=[[TeachLogDetailVC alloc] init];
...
-(void)filtrateBtnPressed:(NSDictionary *)dic
{
    ...
    [self.filtrateData setObject:[NSString stringWithFormat:@"%d",[[dic objectForKey:@"status"] integerValue]+5] forKey:@"status"];
...
-(void)updateData//undo
{
    [self showLoadingWithBG];
    Http *req=[[Http alloc] init];
    req.socialMethord=@"teachinglog/getUnhandledTeachingLogList.yo";
    ...
    [req startWithBlock:^{
        ...
        if ([[req.m_data valueForKey:@"statusCode"] integerValue]==1000)
        {
            ...
            [self.m_aryData addObjectsFromArray:[req.m_data objectForKey:@"teachinglogs"]];
...
-(void)updateDataForSocial//done
{
    ...
    Http *req=[[Http alloc] init];
    req.socialMethord=@"teachinglog/getHandledTeachingLogList.yo";
    ...
    [req startWithBlock:^{
        ...
        if ([[req.m_data valueForKey:@"statusCode"] integerValue]==1000)
        {
            ...
            [self.m_aryDataSocial addObjectsFromArray:[req.m_data objectForKey:@"teachinglogs"]];
...
-(void)showFiltrateVC
{
    ...
    vc.title=@"练车记录查询";
    vc.m_aryOperate=[NSMutableArray arrayWithObjects:@"已写日志",@"学员未到", nil];
    vc.data=[NSMutableDictionary dictionaryWithObjects:@[@"",@"1",@"",@""] forKeys:@[@"studentname",@"status",@"starttime",@"endtime"]];
    vc.delegate=self;
    [self.navigationController pushViewController:vc animated:YES];
}
-(void)submitStudentNotCome:(NSString*)appointid
{
    [self showLoadingWithBG];
    Http *req=[[Http alloc] init];
    req.socialMethord=@"teachinglog/putStudentTeachingLog.yo";
    [req setParams:@[[[MyFounctions getUserInfo] objectForKey:@"account"],appointid,@"7",@""] forKeys:@[@"mobile",@"appointid",@"status",@"log_desc"]];
    [req startWithBlock:^{
        [self stopLoadingWithBG];
        if ([[req.m_data valueForKey:@"statusCode"] integerValue]==1000)
        {
