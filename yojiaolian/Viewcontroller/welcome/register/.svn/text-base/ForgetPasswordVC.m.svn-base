...
-(void)registerMobile
{
    ...
    [req startWithBlock:^{
        ...
        if ([[req.m_data valueForKey:@"statusCode"] integerValue]==1000)
        {
            [MyFounctions saveUserInfo:[NSDictionary dictionaryWithObjects:@[__BASE64(self.mobile)] forKeys:@[@"account"]]];
...
-(void)sendVerifyCodeForResetPassword
{
    [self showLoadingWithBG];
    Http *req=[[Http alloc] init];
    req.socialMethord=@"certification/getVerificationCode.yo";
...
-(void)resetPassword
{
    [self showLoadingWithBG];
    Http *req=[[Http alloc] init];
    req.socialMethord=@"certification/putForgetPassword.yo";
